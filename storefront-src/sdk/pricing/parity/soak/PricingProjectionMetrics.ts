/**
 * Pricing projection soak metrics aggregator (M8 PR-9).
 */

import type { PricingParityReportRecord } from '../../../../domain/pricing/parity/PricingParityResult';
import {
  buildPricingProjectionSoakMetrics,
  computePricingPercentileLatencies,
  isCriticalPricingOutcome,
  type PricingProjectionSoakInput,
  type PricingProjectionSoakMetrics,
} from '../../../../domain/pricing/parity/soak/PricingProjectionCertificationRules';

export class PricingProjectionMetrics {
  aggregate(reports: readonly PricingParityReportRecord[]): PricingProjectionSoakMetrics {
    return buildPricingProjectionSoakMetrics(this.buildInput(reports));
  }

  buildInput(reports: readonly PricingParityReportRecord[]): PricingProjectionSoakInput {
    ...
    const criticalMismatchCount = reports.filter((report) =>
      isCriticalPricingOutcome(report.outcome)
    ).length;
    ...
    const p95LatencyMs = computePricingPercentileLatencies(latencies, 95);
...
export function createPricingProjectionMetrics(): PricingProjectionMetrics {
  return new PricingProjectionMetrics();
}
