...
} from '../menu/parity/soak/MenuProjectionFactory';
import type { MenuParityReportRecord } from '../../domain/menu/parity/MenuParityResult';
import type { MenuProjectionSoakTelemetryEvent } from '../menu/parity/soak/MenuProjectionTelemetry';

const SOAK_FLAGS: MenuFeatureFlagReader = (flag) =>
  flag === 'FF_MENU_PROJECTION_ENABLED' ||
  flag === 'FF_MENU_PROJECTION_PARITY_ENABLED' ||
  flag === 'FF_MENU_PROJECTION_SOAK_ENABLED';

const FIXED_CLOCK = { now: () => '2026-06-27T14:00:00.000Z' };
const FIXED_UUID = {
  generate: (() => {
    let n = 0;
    return () => `menu-soak-${++n}`;
...
const matchReport = (index: number, durationMs = 25): MenuParityReportRecord => ({
  reportId: `report-${index}`,
  catalogId: `catalog-${index}`,
  outcome: 'MATCH',
  differences: [],
  comparedAt: `2026-06-27T14:00:0${index % 10}.000Z`,
  durationMs,
});

const mismatchReport = (index: number): MenuParityReportRecord => ({
  reportId: `report-bad-${index}`,
  catalogId: `catalog-bad-${index}`,
  ...
  comparedAt: `2026-06-27T14:01:0${index % 10}.000Z`,
  durationMs: 30,
});

const versionMismatchReport = (index: number): MenuParityReportRecord => ({
  reportId: `report-version-${index}`,
  catalogId: `catalog-version-${index}`,
  ...
  comparedAt: `2026-06-27T14:02:0${index % 10}.000Z`,
  durationMs: 35,
});

describe('Menu catalog projection soak (M7 PR-9)', () => {
  it('defaults FF_MENU_PROJECTION_SOAK_ENABLED to off', () => {
    assert.equal(
      MENU_SDK_FEATURE_FLAG_DEFAULTS.flags.FF_MENU_PROJECTION_SOAK_ENABLED,
      false
    );
  });

  it('runSoak skips when flags are off', async () => {
    const source = new InMemoryMenuParityReportSource();
    source.seedMany(Array.from({ length: 10 }, (_, index) => matchReport(index)));

    const soak = createMenuProjectionSoakInfrastructure({
      reportSource: source,
      clock: FIXED_CLOCK,
      uuid: FIXED_UUID,
    });
  ...
  it('requires all three menu projection flags including soak flag', async () => {
    const twoFlagsOnly: MenuFeatureFlagReader = (flag) =>
      flag !== 'FF_MENU_PROJECTION_SOAK_ENABLED' && SOAK_FLAGS(flag);
  ...
  it('generates metrics from parity reports', async () => {
    ...
    assert.ok(metrics.value.mismatchDistribution.itemCount >= 2);
  });

  it('runSoak produces READY certification for high parity', async () => {
    const telemetry: MenuProjectionSoakTelemetryEvent[] = [];
    const source = new InMemoryMenuParityReportSource();
    ...
    assert.ok(telemetry.some((event) => event.type === 'menu_projection_soak_started'));
    assert.ok(telemetry.some((event) => event.type === 'menu_projection_readiness_generated'));
    assert.ok(telemetry.some((event) => event.type === 'menu_projection_certification_generated'));
    assert.ok(telemetry.some((event) => event.type === 'menu_projection_soak_completed'));
  });
  ...
  it('detects improving trend from window parity percents', async () => {
    const reports: MenuParityReportRecord[] = [];
