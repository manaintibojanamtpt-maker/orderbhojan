/**
 * Menu projection soak thresholds (M7 PR-9).
 * Pure domain — no infrastructure imports.
 */

export interface MenuProjectionSoakThresholds {
...
export const DEFAULT_MENU_PROJECTION_SOAK_THRESHOLDS: MenuProjectionSoakThresholds = {
...
export const CRITICAL_MENU_PARITY_OUTCOMES = [
  'VERSION_MISMATCH',
  'MISSING_IN_PROJECTION',
] as const;

export function mergeMenuProjectionSoakThresholds(
  overrides: Partial<MenuProjectionSoakThresholds> = {}
): MenuProjectionSoakThresholds {
  return { ...DEFAULT_MENU_PROJECTION_SOAK_THRESHOLDS, ...overrides };
}
