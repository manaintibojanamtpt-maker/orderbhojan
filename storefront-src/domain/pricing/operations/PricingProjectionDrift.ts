/** Pricing projection drift detection (M8 PR-10). Pure domain — no SDK imports. */

export interface PricingProjectionDriftSample {
...
export interface PricingProjectionDriftReport {
  readonly projectionName: string;
...
export function detectPricingProjectionDrift(
  sample: PricingProjectionDriftSample,
  maxDuplicatePercent: number,
  maxDroppedEventPercent: number,
  maxCriticalDriftCount = 0
): PricingProjectionDriftReport {
