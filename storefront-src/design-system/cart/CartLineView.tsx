    ...
    <li>
      <GlassCard hoverEffect={false} className="!rounded-2xl !p-3">
        <div className="grid grid-cols-[auto_1fr_auto] items-start gap-3">
          <div
            className="flex h-[4.5rem] w-[4.5rem] shrink-0 items-center justify-center rounded-xl bg-[#FF7A00]/10 text-[#FF7A00]/70"
            aria-hidden
          >
            <UtensilsCrossed className="h-7 w-7" />
          </div>
          <div className="min-w-0 flex flex-col gap-1">
            <p className="font-bold leading-tight text-white">{line.name}</p>
            {line.variantLabel ? (
              <p className="text-sm text-white/60">{line.variantLabel}</p>
            ) : null}
            {addons.length > 0 ? (
              <div className="mt-0.5 flex flex-wrap gap-1">
                {addons.map((addon) => (
                  <span
                    key={addon.id}
                    className="rounded-full bg-[#FF7A00]/10 px-2 py-0.5 text-[11px] font-semibold text-white/60"
                  >
                    + {addon.label}
            ...
            {line.instructions?.trim() ? (
              <p className="text-sm italic text-white/60">Note: {line.instructions.trim()}</p>
            ) : null}
